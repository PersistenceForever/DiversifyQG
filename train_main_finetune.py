...
def train_forward(args, iteration):
    ...
    for i in range(num_train_epochs):
        ...
        out, semantic_score, diversity_score, val_loss = test_forward(model, tokenizer, args, iteration)  
        val_losses.append(val_loss)
        earlyStop(epoch_loss, model, args, name, iteration)  
        
        with open(os.path.join(args.output_dir, "validation_score.txt"), 'a') as file: 
            file.write(f"{i+1} {semantic_score} {diversity_score}\n")  
        ...
        if(earlyStop.early_stop): ####early stop#####
            print(f"Training stopped early due to early stopping, epoches: {i}" )
            break
    
    ##### Loss Curve######
    plt.figure(figsize=(12, 6))
    plt.plot(range(1, i+2), train_losses, label="Training Loss")
    plt.plot(range(1, i+2), val_losses, label="Validation Loss", linestyle="--")
    ...
    plt.legend()
    plt.grid(True)
    loss_path = os.path.join(args.output_dir, f"loss_curve_{iteration}.png")
    plt.savefig(loss_path, dpi=300)  
    plt.close()  

    
    #### if iteration == last iteration, then finetune####
    if iteration % (args.N - 1) == 0 and iteration > 0: 
        train_pt = os.path.join(args.output_dir, 'forward_raw_train.pt')
        train_loader = DataLoader(train_pt, args.batch_size, training=True)
...
def train_backward(args, iteration):
    ...
    if iteration == 0:
        ...
        forward_model.load_state_dict(torch.load(os.path.join(args.output_dir, 'param_forward_WQ_' + str(iteration) +'.pt')) ) #### previous is str(iteration)
...
def test_forward(model, tokenizer, args, iteration):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, total=len(test_loader)):  
            ...
            batch = batch[:3]
            source_ids, source_mask, y= [x for x in batch]
            y_ids = y[:, :-1].contiguous()
            lm_labels = y[:, 1:].clone()            
            lm_labels[y[:, 1:] == pad_token_id] = -100    
    ...
    out = np.array(outs)
    return out, semantic_similarity, diversity_score, val_loss
...
def main():
    ...
    parser.add_argument('--N', default=7, type=int, help='iteration numbers, WQ is 3 and PQ is 2')
    parser.add_argument('--percent', default=0.8, type=float, help='filter rate')
    parser.add_argument('--num_finetune_epochs', default=10, type = int)
